...
class DiagramAssociationTestCase(TestCase):
    ...
    def test_diagramassociation_creation(self):
        ...
        diagramassociation = DiagramAssociation(
            diagram=Diagram.objects.first(), assigned_object_type=device_ct, assigned_object_id=device.id
        )
    ...
    def test_diagramassociation_creation_with_multiple_devices(self):
        ...
        diagramassociation_1 = DiagramAssociation(
            diagram=Diagram.objects.first(), assigned_object_type=device_ct, assigned_object_id=device_1.id
        )
        diagramassociation_1.full_clean()
        diagramassociation_1.save()

        diagramassociation_2 = DiagramAssociation(
            diagram=Diagram.objects.first(), assigned_object_type=device_ct, assigned_object_id=device_2.id
        )
