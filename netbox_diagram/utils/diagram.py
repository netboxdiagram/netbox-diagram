...
def compute_diagram_data(diagram_id):
    ...
    def add_circuit_node_if_missing(circuit_name, connected_device_names):
        ...
        x, y = (
            (int(sum(x for x, y in coords) / len(coords) + 50), int(sum(y for x, y in coords) / len(coords) + 50))
            if coords
            else (0, 0)
        )
        nodes.append({"name": circuit_name, "association_id": None, "x": x, "y": y, "type": "circuit"})
    ...
    for cable in Cable.objects.prefetch_related("terminations__termination"):
        ...
        for i in range(len(terms)):
            for j in range(i + 1, len(terms)):
                ...
                elif (iface_a and isinstance(terms[j].termination, CircuitTermination)) or (
                    iface_b and isinstance(terms[i].termination, CircuitTermination)
                ):
                    iface = iface_a if iface_a else iface_b
                    cterm = (
                        terms[j].termination
                        if isinstance(terms[j].termination, CircuitTermination)
                        else terms[i].termination
                    )
